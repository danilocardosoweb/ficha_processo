...
async function parseWorkbook(file: File) {
  ...
  const main = sheets.find((sheet) =>
    sheet.rows.some(
      (row) =>
        value(row, "matriz") != null && value(row, "seq", "sequencia") != null,
    ),
  );
  if (!main)
    throw new Error("Não encontrei as colunas Matriz e Seq no relatório.");

  const availability = new Map<string, boolean>();
  for (const sheet of sheets) {
    for (const row of sheet.rows) {
      const status = text(
        value(row, "statusdaferram", "statusdaferramenta", "tipo"),
      );
      const available = value(row, "disponivel", "ativo");
      if (status && available != null)
        availability.set(normalize(status), bool(available));
    }
  }

  const mapped = main.rows
    .map((row, index) => {
      ...
      const sourceStatus = text(
        value(row, "statusdaferram", "statusdaferramenta"),
      );
      const available =
      ...
      const carcassCode =
        packageMeasureMm && carcassDiameterMm
          ? `${dimension(carcassDiameterMm)}X${dimension(packageMeasureMm)}`
          : null;
      const payload = {
      ...
      return {
        ...
        sequence_number: sequence,
        holes: payload.holes,
        theoretical_linear_weight_kg_m: payload.theoretical_linear_weight_kg_m,
        useful_life_kg: payload.useful_life_kg,
        produced_kg: producedKg,
        remaining_kg: payload.remaining_kg,
        source_status: payload.source_status,
        source_available: available,
        machine_codes: payload.machine_codes,
        package_measure_mm: packageMeasureMm,
        carcass_diameter_mm: carcassDiameterMm,
        carcass_code: carcassCode,
        payload,
  ...
  if (!mapped.length)
    throw new Error("O relatório não possui linhas válidas de ferramentas.");
  return mapped;
}
  ...
  async function read(file?: File) {
    ...
    try {
      if (!/\.(xlsx|xls|xlsm|xlsb)$/i.test(file.name))
        throw new Error("Use um arquivo Excel XLSX, XLS, XLSM ou XLSB.");
      ...
      >
        ...
        >
          <SheetHeader className="border-b px-6 py-5 pr-16">
            <SheetTitle>Importar histórico de ferramentas</SheetTitle>
            <SheetDescription>
              Use o relatório F_Historico_Ferramentas. A combinação Matriz +
              Seq. identifica cada ferramenta física.
            </SheetDescription>
            <Button
              type="button"
              variant="ghost"
          ...
          <div className="space-y-5 p-6">
            ...
            <button
              ...
              className="flex min-h-36 w-full flex-col items-center justify-center rounded-xl border border-dashed border-orange-200 bg-orange-50/40 text-center hover:bg-orange-50 disabled:opacity-60"
            >
              {reading ? (
                <Loader2 className="size-8 animate-spin text-orange-500" />
              ) : (
                <FileSpreadsheet className="size-8 text-orange-500" />
              )}
              <strong className="mt-3 text-sm">
                {fileName || "Selecionar F_Historico_Ferramentas.xlsx"}
              </strong>
              <span className="mt-1 text-xs text-slate-500">
                Matriz, Seq, Furos, Medida Pacote, Diâmetro, vida útil e
                histórico
              </span>
            </button>
            {error && (
            ...
            {done && (
              <div
                role="status"
                className="flex gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700"
              >
                <CheckCircle2 className="mt-0.5 size-4 shrink-0" />
                {rows.length.toLocaleString("pt-BR")} registros importados ou
                atualizados.
              </div>
            )}
            {rows.length > 0 && (
              <>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {rows.length.toLocaleString("pt-BR")} registros válidos
                      encontrados.
                    </p>
                  </div>
                  ...
                  <table className="w-full min-w-[900px] text-left text-xs">
                    <thead className="sticky top-0 bg-slate-50 text-[10px] uppercase text-slate-500">
                      <tr>
                        ...
                        <th className="px-3 py-2">Medida pacote</th>
                        <th className="px-3 py-2">Diâmetro</th>
                        <th className="px-3 py-2">Carcaça normalizada</th>
                        <th className="px-3 py-2">Produzido</th>
                        <th className="px-3 py-2">Prensas</th>
                    ...
                    <tbody>
                      {rows.slice(0, 100).map((row) => (
                        <tr key={row.code} className="border-t">
                          ...
                          <td className="px-3 py-2">{row.sequence_number}</td>
                          <td className="px-3 py-2">{row.holes ?? "—"}</td>
                          <td className="px-3 py-2">
                            {row.package_measure_mm?.toLocaleString("pt-BR") ??
                              "—"}{" "}
                            mm
                          </td>
                          <td className="px-3 py-2">
                            {row.carcass_diameter_mm?.toLocaleString("pt-BR") ??
                              "—"}{" "}
                            mm
                          </td>
                          ...
                          <td className="px-3 py-2">
                            {row.produced_kg?.toLocaleString("pt-BR") ?? "—"} kg
                          </td>
                          <td className="px-3 py-2">
                            {row.machine_codes || "—"}
                          </td>
                          <td
                            ...
                            title={row.source_status || ""}
                          >
                            {row.source_status ||
                              (row.source_available ? "Disponível" : "Inativa")}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                {importing && (
                  <div>
                    <div className="mb-2 flex justify-between text-xs text-slate-500">
                      <span>Gravando no cadastro...</span>
                      <span>{progress}%</span>
                    </div>
                ...
                >
                  {importing ? (
                    ...
                    <>
                      Importar {rows.length.toLocaleString("pt-BR")} ferramentas
                    </>
                  )}
                </Button>
              </>
