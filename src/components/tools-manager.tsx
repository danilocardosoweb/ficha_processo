...
const toolName = (tool: Tool) =>
  tool.matrix_code || tool.description?.replace(/^Perfil\s+/i, "") || tool.code;
const statusLabel = (tool: Tool) =>
  tool.source_status ||
  ({
...
export function ToolsManager() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (loadError) {
        if (
          !(await showCached(
            `Sem conexão com o banco. Exibindo o cadastro salvo neste computador. (${loadError.message})`,
          ))
        )
      ...
      if (
        !(await showCached(
          "Modo offline: exibindo o cadastro salvo neste computador.",
        ))
      )
        setError(
          "Não foi possível conectar ao Supabase e ainda não há uma cópia local.",
        );
    } finally {
      setLoading(false);
    }
  ...
  function openForm(tool: Tool | null) {
    setEditing(tool);
    setError("");
    setMessage("");
    setFormOpen(true);
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    const matrixCode = String(form.get("matrix_code") ?? "")
      .trim()
      .toUpperCase();
    const sequence = Number(form.get("sequence_number")) || 0;
    const packageMeasureMm = Number(form.get("package_measure_mm")) || null;
    const carcassDiameterMm = Number(form.get("carcass_diameter_mm")) || null;
    const payload = {
      organization_id: organizationId,
      code:
        editing?.code ||
        `MANUAL:${matrixCode}:${String(sequence).padStart(3, "0")}`,
      matrix_code: matrixCode,
      sequence_number: sequence,
      description:
        String(form.get("description") ?? "").trim() ||
        `Matriz ${matrixCode} · Seq. ${sequence}`,
    ...
    const query = editing
      ? createClient().from("tools").update(payload).eq("id", editing.id)
      : createClient().from("tools").insert(payload);
    try {
      const { error: saveError } = await withSupabaseTimeout(query);
      if (saveError)
        setError(
          saveError.code === "23505"
            ? "Já existe esta ferramenta e sequência."
            : saveError.message,
        );
      else {
        setMessage(
          editing ? "Ferramenta atualizada." : "Ferramenta cadastrada.",
        );
        setFormOpen(false);
        setEditing(null);
        requestOfflineSync("tools");
        await load();
      }
    } catch {
      setError(
  ...
  return (
    <section className="rounded-xl border bg-white shadow-sm">
      <div className="flex flex-col gap-4 border-b p-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="font-heading font-bold">Ferramentas cadastradas</h2>
          <p className="mt-1 text-xs text-slate-500">
            {total.toLocaleString("pt-BR")} ferramenta(s) física(s). Dados do
            histórico Excel, separados por matriz e sequência.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 border-b p-4 lg:flex-row">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-9"
            placeholder="Buscar ferramenta (ex.: DIN7501 ou DIN-7501)..."
          />
        </div>
        <Button
          type="button"
        ...
        <>
            ...
            <table className="w-full min-w-[1240px] text-left text-sm">
              <thead className="sticky top-0 z-10 bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500 shadow-[0_1px_0_0_#e2e8f0]">
                <tr>
                  ...
                  <th className="px-4 py-3">Furos</th>
                  <th className="px-4 py-3">Carcaça</th>
                  <th className="px-4 py-3">Medida pacote</th>
                  <th className="px-4 py-3">Diâmetro</th>
                  <th className="px-4 py-3">Peso linear</th>
                  <th className="px-4 py-3">Produzido / vida útil</th>
                  <th className="px-4 py-3">Prensas</th>
              ...
              <tbody>
                {tools.map((tool) => (
                  <tr key={tool.id} className="border-t hover:bg-orange-50/40">
                    ...
                    <td className="px-4 py-3">{tool.holes ?? "—"}</td>
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-violet-50 px-2.5 py-1 font-mono text-xs font-black text-violet-700">
                        {tool.carcass_code || "—"}
                      </span>
                    </td>
                    <td className="px-4 py-3 tabular-nums">
                      {tool.package_measure_mm?.toLocaleString("pt-BR") ?? "—"}{" "}
                      mm
                    </td>
                    <td className="px-4 py-3 tabular-nums">
                      {tool.carcass_diameter_mm?.toLocaleString("pt-BR") ?? "—"}{" "}
                      mm
                    </td>
                    <td className="px-4 py-3 tabular-nums">
                      {(
                        tool.theoretical_linear_weight_kg_m ??
                        tool.actual_linear_weight_kg_m
                      )?.toLocaleString("pt-BR") ?? "—"}{" "}
                      kg/m
                    </td>
                    <td className="px-4 py-3 tabular-nums">
                      <strong>
                        {(tool.produced_kg ?? tool.lifecycle_kg).toLocaleString(
                          "pt-BR",
                        )}{" "}
                        kg
                      </strong>
                      <span className="text-slate-400">
                        {" "}
                        / {tool.useful_life_kg?.toLocaleString("pt-BR") ??
                          "—"}{" "}
                        kg
                      </span>
                    </td>
                    <td className="px-4 py-3">{tool.machine_codes || "—"}</td>
                    <td className="px-4 py-3">
                      <span
                        title={tool.source_status || ""}
                        className={
                          (tool.source_available ?? tool.status === "available")
                            ? "inline-block max-w-48 truncate rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-semibold text-emerald-700"
                            : "inline-block max-w-48 truncate rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600"
                        }
                      >
                        {statusLabel(tool)}
                      </span>
          ...
          <div className="flex items-center justify-between border-t p-4 text-xs text-slate-500">
            <span>
              ...
              {Math.min((page + 1) * pageSize, total)} de{" "}
      ...
      <Sheet open={formOpen} onOpenChange={setFormOpen}>
        ...
        >
          <SheetHeader className="border-b px-6 py-5 pr-16">
            ...
            <SheetDescription>
              Cadastro físico por matriz, sequência e carcaça requerida.
            </SheetDescription>
            <Button
              type="button"
              variant="ghost"
          ...
          >
            <div className="grid grid-cols-[1fr_110px] gap-4">
              <Field
                label="Ferramenta / matriz"
                name="matrix_code"
                defaultValue={editing ? toolName(editing) : ""}
                required
              />
              <Field
                label="Sequência"
                ...
                type="number"
                min="0"
                defaultValue={editing?.sequence_number ?? 0}
                required
              />
            </div>
            <Field
            ...
            <div className="rounded-xl border border-violet-100 bg-violet-50/50 p-4">
              <p className="mb-3 text-xs font-bold text-violet-800">
                Carcaça = Diâmetro × Medida pacote
              </p>
              <div className="grid grid-cols-2 gap-4">
                <Field
                  label="Medida pacote (mm)"
                  name="package_measure_mm"
                  type="number"
                  step="0.01"
                  min="0"
                  defaultValue={editing?.package_measure_mm ?? ""}
                />
                <Field
                  label="Diâmetro (mm)"
                  name="carcass_diameter_mm"
                  type="number"
                  step="0.01"
                  min="0"
                  defaultValue={editing?.carcass_diameter_mm ?? ""}
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <Field
                ...
                type="number"
                min="0"
                defaultValue={editing?.holes ?? ""}
              />
              <Field
                label="Peso linear (kg/m)"
                name="theoretical_linear_weight_kg_m"
                type="number"
                step="0.0001"
                min="0"
              ...
              <Field
                ...
                min="0"
                defaultValue={
                  editing?.produced_kg ?? editing?.lifecycle_kg ?? 0
                }
              />
              <Field
                label="Vida útil (kg)"
                ...
                step="0.001"
                min="0"
                defaultValue={editing?.useful_life_kg ?? ""}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="tool-status">Situação</Label>
              <select
                id="tool-status"
                name="status"
            ...
            <Button
              type="submit"
              className="h-11 w-full bg-orange-500 font-semibold hover:bg-orange-600"
              disabled={saving}
            >
              {saving ? (
              ...
              {editing ? "Salvar alterações" : "Cadastrar ferramenta"}
            </Button>
          </form>
        </SheetContent>
      </Sheet>
