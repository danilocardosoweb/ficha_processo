...
import { CheckCircle2, KeyRound, Pencil, Plus, Search, UsersRound, XCircle } from "lucide-react";
...
export function UsersManager({ initialUsers, currentUserId }: { initialUsers: ManagedUser[]; currentUserId: string }) {
  ...
  async function reset(event: FormEvent) {
    ...
    setResetting(null); setMessage("Senha temporária definida. As sessões anteriores foram encerradas."); await refresh();
  }
  ...
  return <>
    ...
    <Dialog open={resetting !== null} onOpenChange={(open) => !open && setResetting(null)}><DialogContent><DialogHeader><DialogTitle>Redefinir senha</DialogTitle><DialogDescription>{resetting?.display_name} será desconectado e deverá trocar a senha no próximo acesso.</DialogDescription></DialogHeader><form onSubmit={reset} className="space-y-4"><label className="text-sm font-semibold">Nova senha temporária<div className="mt-1 flex gap-2"><Input className="h-10" value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} required /><Button type="button" variant="outline" onClick={() => setResetPassword(generatePassword())}>Gerar</Button></div></label>{error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}<DialogFooter><Button type="button" variant="outline" onClick={() => setResetting(null)}>Cancelar</Button><Button type="submit" disabled={saving}>Redefinir senha</Button></DialogFooter></form></DialogContent></Dialog>
  </>;
}
