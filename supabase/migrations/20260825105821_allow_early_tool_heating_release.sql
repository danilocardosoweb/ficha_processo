-- Permite a retirada antecipada com justificativa obrigatória e auditoria explícita.
alter table public.tool_heating_cycles
  add column if not exists released_early boolean not null default false,
  add column if not exists actual_heating_minutes integer;

alter table public.tool_heating_cycles
  drop constraint if exists tool_heating_cycles_actual_minutes_check;
alter table public.tool_heating_cycles
  add constraint tool_heating_cycles_actual_minutes_check
  check (actual_heating_minutes is null or actual_heating_minutes >= 0);
